...
// As a result, the app sends the DeepStream Minimal metadata payloads on demand.
func (dsh *DeepstreamMinimalHandler) ProcessingHandle(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		http.Error(w, "Failed to load DeepStream Minimal metadata", http.StatusInternalServerError)
		return
	}
	log.Printf("Sending metadata")
	err = dsh.TopicRestService.SendData(eventData.CameraID, eventData.StreamID, eventData.TopicName, enums.Metadata, "json", jsons)
	if err != nil {
		log.Printf("Error sending the DeepStream Minimal metadata: %s", err)
		http.Error(w, "Failed to send DeepStream Minimal metadata", http.StatusBadGateway)
		return
	}
